model = dict(
    type='CascadeRCNN',
    # pretrained='/opt/ml/detection/Swin-Transformer-Object-Detection/swin_large_patch4_window7_224_22k.pth',
    pretrained=None,
    backbone=dict(
...
data = dict(
    ...
    train=dict(
        type='CocoDataset',
        ann_file='../dataset/train_pseudo.json',
        img_prefix='../dataset/',
        pipeline=[
    ...
    val=dict(
        type='CocoDataset',
        ann_file='../dataset/val_split.json',
        img_prefix='../dataset/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
                type='MultiScaleFlipAug',
                img_scale=[(512, 512), (1024, 1024), (1536, 1536)],
                flip=True,
                transforms=[
...
runner = dict(type='EpochBasedRunnerAmp', max_epochs=4)
...
log_level = 'INFO'
# load_from = None
load_from = './work_dirs/swin/large_224_ms_50/latest.pth' # 훈련시킨 weight
resume_from = None
workflow = [('train', 1)]
seed = 2021
fp16 = None
work_dir = './work_dirs/swin/large_224_ms_50_pseudo'
gpu_ids = range(0, 1)
